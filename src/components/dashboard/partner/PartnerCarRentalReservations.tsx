...
import { 
  ...
  updatePartnerCarRentalReservationStatus, 
  deletePartnerCarRentalReservation,
  PartnerCarRentalReservation 
} from "@/services/partnerCarRentalService";
import { Trash2, Eye, CheckCircle, XCircle, Clock, Car } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export const PartnerCarRentalReservations = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [selectedReservation, setSelectedReservation] = useState<PartnerCarRentalReservation | null>(null);
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) => 
      updatePartnerCarRentalReservationStatus(id, status),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['partner-car-rental-reservations'] });
      toast({
  ...
  const deleteMutation = useMutation({
    mutationFn: deletePartnerCarRentalReservation,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['partner-car-rental-reservations'] });
      toast({
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Réservations de Location de Voitures
          </CardTitle>
          <div className="text-sm text-gray-600">
            Gérez toutes les réservations de vos véhicules
          </div>
        </CardHeader>
        <CardContent>
